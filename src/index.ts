...
const getColLookup = (shName: string, logger: InstanceType<typeof LogAccumulator>) => {

  const lookup: Record<string, number> = {};

  try {
    const sheet = getSheet(shName);

    const headers = getRowVals(sheet, 1);

    headers.forEach((header, hi) => (lookup[header] = hi));
  } catch (error) {
    logger.warn(`failed ${shName} lookup: ${error}`);
  }
...
const getColLookupUS = (logger: InstanceType<typeof LogAccumulator>) =>
  getColLookup(CONFIG.sheets.country, logger);

const getColLookupState = (logger: InstanceType<typeof LogAccumulator>) =>
  getColLookup(CONFIG.sheets.states, logger);

const testColLookup = () => {
  console.log(getColLookupUS());
  console.log( getColLookupState() );
};